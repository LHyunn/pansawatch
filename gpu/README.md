...
PansaWatch는 GPU 서버의 `/home/hyun/pansawatch/` 를 프로젝트 root로 사용 (Linux 사용자는 `<GPU_SSH_USER>`, `hyun`은 디렉토리명).

```bash
# 1. 프로젝트 sync (로컬 → GPU 서버)
rsync -avz -e "ssh -p <GPU_SSH_PORT>" --exclude='node_modules' --exclude='.next' \
  ./ <GPU_SSH_USER>@<GPU_SSH_HOST>:/home/hyun/pansawatch/
