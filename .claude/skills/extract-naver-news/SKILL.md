...
| `GPU_SSH_USER` / `GPU_SSH_HOST` / `GPU_SSH_PORT` | required | SSH endpoint of the GPU server — the script SSH-orchestrates the remote vLLM container (key-based auth) |
...
- **Judge matching coverage** — only matches judges already in `data/judges.json` (2,987 entries from 법원공보 PDFs). Articles about judges not in the dataset are registered but go to `unlinked` (no judge link).
