...
> ⚠️ **상태 안내**: 이 문서는 **설계(계획) 명세**이며, 현재 구현은 이와 다릅니다.
> 아래 기술된 Claude API 기반 요약은 현재 운영 경로가 아니며, 실제로는 일부만 구축된 자체 호스팅(self-hosted) 방식으로 진행 중입니다.
> 따라서 본 문서의 모델·캐시·비용·코드 내용은 향후 목표일 뿐 현 상태를 반영하지 않습니다.
