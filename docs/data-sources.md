...
| 5 | **대법원 보도자료 게시판** (`scourt.go.kr/portal/news`) | 인사발령 | 없음 | 0 KRW | 1차 | ✅ judges 동기화 |
| 6 | **사법정보공유포털** (`openapi.scourt.go.kr`) | 법원 공공데이터 (연계 API) | 별도 신청 (publicapi@scourt.go.kr) | 0 KRW | 3차 (후속) | ❌ Phase 4+ |
| 7 | **대법원 종합법률정보** (`glaw.scourt.go.kr`) | 판례 (대체) | 없음 | 0 KRW | 3차 | ❌ #1 폴백 |
| 8 | **케이스노트** (`casenote.kr`) | 판례 (민간) | 없음 | 0 KRW | 참고만 | ❌ AI 봇 차단 |
| 9 | **공공데이터포털** (`data.go.kr`) | 메타 카탈로그 | 데이터셋별 | 0 KRW | 참고 | 카탈로그 점검용 |
| 10 | **재판관맵** (`saibankan-map.jp`) | 일본 레퍼런스 | — | — | 참고 | 설계 비교용 |
...
관련 게시물 예시 (외부 인용 — [리걸타임즈](https://www.legaltimes.co.kr/news/articleView.html?idxno=91923)):
